  ...
  public QualificationViewModel(ServiceNuits service) {
    ...
    descriptionSelection.bind(
        Bindings.createStringBinding(
            () -> {
              Sequence selectionnee = sequenceSelectionnee.get();
              if (selectionnee == null) {
                return "(sélectionnez une séquence dans le tableau)";
              }
              return String.format(
                  "Séquence %s - %.1f kHz",
                  HEURE.format(selectionnee.getHorodatage()),
                  selectionnee.getFrequenceDominanteKHz());
            },
    ...
    verdictGlobalLibelle.bind(
        Bindings.createStringBinding(
            () -> {
              ...
              return verdict == null || verdict.isBlank()
